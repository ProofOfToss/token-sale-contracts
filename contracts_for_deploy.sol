...
contract RefundVault{
    ...
    function close(address _wallet1, address _wallet2, uint256 _feesValue) public;
    function enableRefunds() public;
    function refund(address investor) public;
    function restart() external;
    function del(address _wallet) external;
}

contract FeesStrategy{
    uint256[2] public amount;
    function updateAmont(uint256[2] _amount) external;
    function pay(uint256 _value) external returns (uint256);
...
contract Creator{
    ...
    function createFeesStrategy() external returns(FeesStrategy);
}
*/
...
contract Token is FreezingToken, MintableToken, MigratableToken, BurnableToken {
    ...
    */
    ...
    function transferToContract(address to, uint value, bytes _data) public contractOnly(_to) returns (bool) {
    ...
    //!!! Добавил модификатор public
    function allowBlocking(address owner, address contract) contractOnly(_contract) public {

        //!!! Заменил if(condition) throw на require

        require(_contract != msg.sender && contract != owner);

        require(grantedToAllowBlocking[_owner][msg.sender]);

        allowedToBlocking[_owner][_contract] = true;

        emit TokenOperationEvent('allow_blocking', owner, contract, 0, msg.sender);
    }
    ...
    //!!! Добавил модификатор public
    //!!! Внимание! Гет метод для grantedToAllowBlocking избыточен! Возможно следует убрать public модификатор для grantedToAllowBlocking?
    function allowanceToAllowBlocking(address owner, address contract) public constant returns (bool granted) {
    ...
    function block(address blocking, uint256 value) whenNotPaused(_blocking) public {
        //!!! Заменил if(condition) throw на require
        require(allowedToBlocking[_blocking][msg.sender]);

        //!!! Внимание! Нет проверки на замороженные токены. Если токены на счкту _blocking заморожены, они будут списаны игнорируя заморозку! Так и должно быть?
        require(balances[_blocking] >= value && value > 0);

        balances[_blocking] -= _value;
        blocked[_blocking][msg.sender] += _value;

        emit TokenOperationEvent('block', blocking, 0, value, msg.sender);
    }
    ...
    //!!! Добавил модификатор public
    function unblock(address blocking, address unblockTo, uint256 _value) whenNotPaused(_unblockTo) public {
        //!!! Заменил if(condition) throw на require
        require(allowedToBlocking[_blocking][msg.sender]);
        require(blocked[_blocking][msg.sender] >= value && value > 0);

        blocked[_blocking][msg.sender] -= _value;
        balances[_unblockTo] += _value;

        emit TokenOperationEvent('unblock', blocking, unblockTo, _value, msg.sender);
    }
}
...
/*
contract Crowdsale {
    enum Roles {beneficiary, accountant, manager, observer, bounty, company, team, founders, found, fees}
    enum TokenSaleType {round1, round2}
    address[10] public wallets;
    TokenSaleType public TokenSale;
    function weiRaised() public constant returns(uint256);
